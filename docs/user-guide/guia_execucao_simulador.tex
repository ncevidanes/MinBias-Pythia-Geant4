...
\section{Reprodutibilidade, BCID e número de threads}

\subsection{Estado validado até o Ciclo 11}

O Ciclo 11 estabelece o \texttt{bcid} como identidade científica global do
bunch crossing. O identificador \texttt{event} permanece local ao job e pode
reiniciar em zero quando uma campanha é particionada.

A geração primária utiliza uma identidade de seed baseada em:

\begin{lstlisting}[style=config]
(seed_base, bcid, subevent, stream)
\end{lstlisting}

O transporte Geant4 utiliza um stream específico por evento:

\begin{lstlisting}[style=config]
(seed_base, bcid, transport-event stream)
\end{lstlisting}

O seed de transporte é instalado em
\texttt{EventAction::BeginOfEventAction}, depois da geração das primárias e
antes do tracking.

A matriz do Ciclo 11 demonstrou igualdade científica canônica para:

\begin{itemize}
  \item execução monolítica versus jobs particionados;
  \item rerun exato;
  \item shards executados em ordem física invertida;
  \item uma versus duas threads;
  \item shards com duas threads;
  \item topologias mistas 1T/2T;
  \item número fixo de interações;
  \item interações Poisson;
  \item beam smearing não nulo.
\end{itemize}

Portanto, dentro da matriz validada, o número de threads é um parâmetro
operacional e de desempenho, mas não participa da identidade científica do RNG.

O contrato científico validado é:

\begin{lstlisting}[style=config]
(seed_base, bcid, physics configuration)
    -> mesmo resultado científico canônico
\end{lstlisting}

independentemente da partição, da ordem dos jobs, do reinício do
\texttt{eventId} local e da topologia 1T/2T cobertas pela validação.

Seeds-base distintas continuam gerando realizações físicas distintas.

As evidências estão documentadas em:

\begin{lstlisting}[style=shell]
docs/cycle-11-partition-stable-production/
\end{lstlisting}
...
\begin{enumerate}
  ...
  \item registre o número de threads como parâmetro operacional;
...
Este guia descreve a interface operacional do simulador e foi atualizado
com a política de reprodutibilidade validada no Ciclo 11.
